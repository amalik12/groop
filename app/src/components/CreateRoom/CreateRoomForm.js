...
let CreateRoomForm = (props) => {
    return (
        <div className="CreateRoomForm">
            ...
            <TextField large={true} float={false} errorText={props.name_error} label={NAME_LABEL} value={props.name} handleChange={props.handleChange} />
            <span className="create-room-domain">{window.location.host + '/'}</span>
            <TextField large={true} inline={true} float={false} label={SHORTID_LABEL}
